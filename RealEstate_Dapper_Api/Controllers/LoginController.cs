...
namespace RealEstate_Dapper_Api.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<IActionResult>SignIn(CreateLoginDto loginDto)
        {
            ...
            using (var connecection=_context.CreateConnection())
            {
                var values=await connecection.QueryFirstOrDefaultAsync<CreateLoginDto>(query, parameters);
                var values2=await connecection.QueryFirstAsync<GetAppUserIdDto>(query2, parameters);
                if (values!=null)
                {
                    GetCheckAppUserViewModel model = new GetCheckAppUserViewModel();
                    model.UserName = values.Username;
                    model.Id = values2.UserId;
                    var token = JvtTokenGenerator.GenerateToken(model);
                    return Ok(token);
                }
                else
                {
